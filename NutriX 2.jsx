...
// Calls YOUR OWN backend function (deployed on Supabase), which safely holds your real
// Anthropic API key and forwards the request. This is required — calling Anthropic
// directly from the browser (like this used to) does not work on a real website.
...
/* Main App                                                             */
/* ================================================================== */
// Local calendar date as "YYYY-MM-DD", not UTC — so the day changes at your actual
// midnight, not some other timezone's.
function todayStr() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

// If the saved data is from a previous day, this clears today's log/water and
// updates the streak: +1 if you logged yesterday (consecutive), reset to 1 if you
// missed a day entirely. If it's still the same day, nothing changes.
function applyDayRollover(blob) {
  const today = todayStr();
  if (!blob.lastLogDate) {
    // No record of when this account last logged (it predates day-tracking being added) —
    // safest is to treat old totals as stale and start fresh, rather than leave them stuck forever.
    return { ...blob, todayLog: [], water: 0, streak: 1, lastLogDate: today };
  }
  const last = blob.lastLogDate;
  if (last === today) return { ...blob, lastLogDate: today };
  const lastDate = new Date(last + "T00:00:00");
  const todayDate = new Date(today + "T00:00:00");
  const diffDays = Math.round((todayDate - lastDate) / 86400000);
  const newStreak = diffDays === 1 ? (blob.streak || 1) + 1 : 1;
  return { ...blob, todayLog: [], water: 0, streak: newStreak, lastLogDate: today };
}
...
const emptyAccountState = (name) => ({
  ...
  weightHistory: [], onboarded: false, lastLogDate: todayStr(),
});
...
export default function NutriXApp() {
  ...
  const [lastLogDate, setLastLogDateState] = useState(todayStr());
  ...
  function loadAccountIntoState(accRaw) {
    const acc = applyDayRollover(accRaw); // resets today's log/water and updates streak if a new day has started
    ...
    setWeightHistory(acc.weightHistory || []); setLastLogDateState(acc.lastLogDate || todayStr());
  }
  ...
  useEffect(() => {
    ...
    const blob = {
      ...
      unlockedBadges: Array.from(unlockedBadges), weightHistory, onboarded: stage === "main", lastLogDate,
    };
    saveAppData(userId, blob);
  }, [profile, goals, todayLog, water, xp, level, streak, totalLogs, photoLogs, hydrationHits, dessertLogs, snackLogs, beverageLogs, cuisinesTried, unlockedBadges, weightHistory, stage, lastLogDate]); // eslint-disable-line

  // catches the case where the app is left open across midnight (no reload happens,
  // so the load-time rollover check above never runs) — checks again whenever the
  // tab/app becomes visible again.
  useEffect(() => {
    if (stage !== "main") return;
    function checkForNewDay() {
      const today = todayStr();
      if (today === lastLogDate) return;
      const lastDate = new Date(lastLogDate + "T00:00:00");
      const todayDate = new Date(today + "T00:00:00");
      const diffDays = Math.round((todayDate - lastDate) / 86400000);
      setStreak((s) => (diffDays === 1 ? s + 1 : 1));
      setTodayLog([]);
      setWater(0);
      setLastLogDateState(today);
    }
    document.addEventListener("visibilitychange", checkForNewDay);
    checkForNewDay();
    return () => document.removeEventListener("visibilitychange", checkForNewDay);
  }, [stage, lastLogDate]);

  useEffect(() => { if (stage === "main") checkBadges(); }, [stage]); // eslint-disable-line
